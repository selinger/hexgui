...
public final class PrefUtil {
  ...
  /**
   * Get a list of strings from preferences. The list is stored as a size property end element_N
   * properties with N being the element index.
   *
   * @param path The absolute path name of the node.
   ...
   *
   * @param path The absolute path name of the node.
   * @return The node or null, if node does not exist or failure in the backing store.
   */
  public static Preferences getNode(String path) {
    assert !path.startsWith("/");
    Preferences prefs = Preferences.userRoot();
    try {
      if (!prefs.nodeExists(path)) return null;
  ...
  /**
   * Put a list of strings to preferences. The list is stored as a size property end element_N
   * properties with N being the element index.
   *
   * @param path The absolute path name of the node.
  ...
  public static void putList(String path, ArrayList<String> list) {
    ...
    for (int i = 0; i < list.size(); ++i) prefs.put("element_" + i, list.get(i));
  }

...
class ExitWaiter extends Thread {
...
public class ProcessUtil {
  ...
  public static class StdErrThread extends Thread {
    public StdErrThread(Process process) {
      super(new StreamCopy(false, process.getErrorStream(), System.err, false));
    }
  }
  ...
  public static String runCommand(String[] cmdArray) throws IOException {
    ...
    try {
      ...
      try {
        if (process.waitFor() != 0) throw new IOException("Process returned error status");
      } catch (InterruptedException e) {
  ...
  /**
   * Run a process. Forwards the stdout/stderr of the child process to stderr of the calling
   * process.
   */
  public static void runProcess(String[] cmdArray) throws IOException {
    Runtime runtime = Runtime.getRuntime();
    Process process = runtime.exec(cmdArray);
    Thread copyOut = new Thread(new StreamCopy(false, process.getInputStream(), System.err, false));
    copyOut.start();
    Thread copyErr = new Thread(new StreamCopy(false, process.getErrorStream(), System.err, false));
    copyErr.start();
  }

...
/** Static utility functions for platform detection and platform-dependent behavior. */
public class Platform {
  ...
  public interface SpecialMacHandler {
     ...
     *
     * @return true if event was handled successfully, false if quit should be aborted.
     */
    boolean handleQuit();
  }
  ...
  public static String getJavaRuntimeName() {
    ...
    if (name == null) name = System.getProperty("java.vm.name");
    return name;
  }

  /**
   * Return information on this computer. Returns host name and cpu information (if /proc/cpuinfo
   * exists).
   */
  public static String getHostInfo() {
    ...
    try {
      if (existsProcCpuinfo()) {
        String[] cmdArray = {"/bin/sh", "-c", "grep '^model name' /proc/cpuinfo"};
        String result = ProcessUtil.runCommand(cmdArray);
        int start = result.indexOf(':');
        if (start >= 0) {
          info = info + " (";
          int end = result.indexOf("\n");
          if (end >= 0) info = info + result.substring(start + 1, end).trim();
          else info = info + result.substring(start + 1).trim();
          info = info + ")";
        }
      }
    } catch (IOException e) {
  ...
  /**
   * Try to open a URL in an external browser. Tries /usr/bin/open if Platform.isMac(), rundll32
   * url.dll,FileProtocolHandler if Platform.isWindows(), and if isUnix() in this order: - xdg-open
   * - kfmclient (if KDE is running) - firefox - mozilla - opera
   *
  ...
  public static boolean openInExternalBrowser(URL url) {
    if (isMac()) {
      ...
      String[] cmd = {"rundll32", "url.dll,FileProtocolHandler", url.toString()};
      if (runProcess(cmd)) return true;
    } else if (isUnix()) {
      {
  ...
  public static void registerSpecialMacHandler(SpecialMacHandler handler) {
    try {
      ...
      System.err.println(
          "Could not register handler for Mac events." + " (com.apple.eawt classes not found)");
    }
  }
